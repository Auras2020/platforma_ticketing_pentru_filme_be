...
import com.example.platforma_ticketing_be.dtos.SeatFilterDto;
...
@Component
public class SeatSpecificationImpl {

    public Specification<Seat> getSeats(SeatFilterDto dto) {
        return (root, query, builder) -> {
            List<Predicate> predicates = new ArrayList<>();
            List<Predicate> searchPredicatesList = new ArrayList<>();

            Join<Seat, ShowTiming> seatShowTimingJoin = root.join("showTiming", JoinType.INNER);
            Join<ShowTiming, Theatre> showTimingTheatreJoin = seatShowTimingJoin.join("theatre", JoinType.INNER);
            Join<ShowTiming, Movie> showTimingMovieJoin = seatShowTimingJoin.join("movie", JoinType.INNER);
